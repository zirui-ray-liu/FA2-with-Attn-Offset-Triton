...
def _attn_fwd(sm_scale, M,  #
    ...
    if STAGE & 2:
        acc, l_i, m_i = _attn_fwd_inner(acc, l_i, m_i, q,  #
                                        ...
                                        warp_specialize, IS_HOPPER)
    # epilogue
    cur_bias = tl.load(bias_ptr + off_h)
    l_i =  l_i + cur_bias * tl.exp2(-m_i)
...
def _attn_bwd(Q, K, V, bias, sm_scale,  #
    ...
    delta_vec = tl.load(D + offs_m)
    db_block = -tl.sum(delta_vec * tl.exp2(-m_vec), axis=0)
    head_idx = (bhid % H).to(tl.int64)
    tl.atomic_add(DB + head_idx, db_block)


class _attention(torch.autograd.Function):

    @staticmethod
    def forward(ctx, q, k, v, bias, causal, sm_scale, warp_specialize=True):
        # shape constraints
        ...
        _attn_fwd[grid](
            sm_scale, M,  #
            q.shape[0], q.shape[1],  #
            desc_q, desc_k, desc_v, desc_o, bias, #
            N_CTX=q.shape[2],  #
            HEAD_DIM=HEAD_DIM_K,  #
    ...
    def backward(ctx, do):
        ...
        db = torch.zeros_like(bias)   # must be 0 for atomic add correctness
        ...
        _attn_bwd[grid](
            q, arg_k, v, bias,ctx.sm_scale, do, dq, dk, dv, db, #
            M, delta,  #
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, dtype=torch.float16):
    ...
    bias = (torch.empty((H, ), dtype=torch.float32, device=DEVICE).normal_(mean=0.0, std=0.5).requires_grad_())
    bias.data = torch.abs(bias.data) + 0.1  # ensure positive
    # by design, the bias should be positive
    sm_scale = 0.5
    ...
    if causal:
        p[:, :, M == 0] = float("-inf")
    denominator = torch.sum(torch.exp(p), dim=-1, keepdim=True)
    p = torch.softmax(p.float(), dim=-1)
    p = p * denominator / (denominator + bias.view(1, -1, 1, 1))
    lse_ref = torch.log(denominator + bias.view(1, -1, 1, 1)).squeeze(-1) * 1.4426950408889634 # = 1.0 / ln(2)
    p = p.to(ref_dtype)
    # p = torch.exp(p)
    ...
    tri_out = attention(q, k, v, bias, causal, sm_scale, warp_specialize)
    tri_out = tri_out.half()
    if mode == "fwd":
        atol = 3 if "fp8" in provider else 1e-2
        torch.testing.assert_close(tri_out, ref_out, atol=atol, rtol=0)
        # torch.testing.assert_close(lse_ref, lse_out, atol=atol, rtol=0)
        return
    ...
    torch.testing.assert_close(tri_dbias, ref_dbias, atol=1e-2, rtol=rtol)
...
if __name__ == "__main__":
    ...
    test_op(Z=1, H=32, N_CTX=4096, HEAD_DIM=64, causal=True, warp_specialize=False, mode="bwd", provider="triton-fp16")